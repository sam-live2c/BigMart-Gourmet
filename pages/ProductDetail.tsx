...
const ProductDetail: React.FC<ProductDetailProps> = ({
  ...
  return (
    <div className="bg-[#f1f2f4] min-h-screen pb-24 md:pb-0 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-0 md:gap-4 md:py-6 md:px-4">
        ...
        <div className="w-full md:w-7/12 flex flex-col gap-2">
          ...
          <div className="bg-white p-4 flex flex-col gap-4">
            <h1 className="text-[15px] text-gray-800 leading-snug">
              {product.name}
            </h1>

            {/* Price block - visible on desktop, hidden on mobile since it's on bottom bar? The screenshot didn't show main price at top, but usually it's there. */}
            ...
            <div className="mt-2 border-t border-gray-100 pt-4">
              <h3 className="text-sm font-bold text-gray-800 mb-2">Quantity</h3>
              <div className="w-32">
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-3">
                <div className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#15803d]"
                    />
                    <button 
                      onClick={handleCheckPincode}
                      disabled={pincodeStatus === 'checking' || pincode.length !== 6}
                      className="bg-black text-white hover:bg-gray-800 disabled:bg-gray-200 disabled:text-gray-500 font-medium px-4 py-2 rounded-lg text-sm transition"
                    >
                ...
                {pincodeStatus === 'available' && (
                  <p className="text-sm text-[#15803d] font-bold pl-8">
                    Superfast delivery available under 1 hour!
                  </p>
                )}
                {pincodeStatus === 'unavailable' && (
                  <p className="text-sm text-red-600 font-bold pl-8">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 bg-gray-50 p-3 rounded-lg border border-gray-100 flex items-center">
      ...
      {/* COD Confirmation Modal (Amazon-style minimal) */}
      {showCODModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 p-4 animate-in fade-in">
          <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl overflow-hidden scale-in animate-in zoom-in-95 duration-200">
            <div className="p-5">
              <h3 className="font-bold text-xl text-gray-900 mb-2">Confirm Order</h3>
              <p className="text-sm text-gray-600 mb-4">You are placing an order for <strong>{product.name}</strong>. You can pay via Cash or UPI on delivery.</p>
              
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-5 font-sans">
                <div className="flex justify-between items-center mb-1 text-sm font-medium">
                  <span className="text-gray-600">Unit Price:</span>
                  <span className="text-gray-900">₹{(product.price ?? 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center mb-1 text-sm font-medium">
                  <span className="text-gray-600">Quantity:</span>
                  <span className="text-gray-950 font-bold">{quantity}</span>
                </div>
                <div className="flex justify-between items-center mb-1 text-sm font-medium border-t border-gray-200/60 pt-1.5 mt-1.5">
                  <span className="text-gray-900 font-bold">Total Amount:</span>
                  <span className="text-[#15803d] font-bold text-base">₹{((product.price ?? 0) * quantity).toLocaleString()}</span>
                </div>
