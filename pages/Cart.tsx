...
const Cart: React.FC<CartProps> = ({ cart, setCart, wishlist = [], onToggleWishlist, user, onOrderSuccess }) => {
  ...
  const [codProduct, setCodProduct] = useState<CartItem | null>(null);
  ...
  const handleInstantCODClick = (item: CartItem) => {
    setCodProduct(item);
  };

  const confirmCOD = () => {
    if (codProduct) {
      const orderAmount = getItemPrice(codProduct);
      const order: Order = {
                 ...
                 enrichedCart.map(item => (
                    <div key={item.id} className="bg-white md:rounded-xl shadow-sm overflow-hidden border-y md:border-x md:border-gray-200">
                      ...
                      <div className="flex border-t border-gray-200 bg-white">
                        ...
                        <button onClick={() => handleInstantCODClick(item)} className="flex-1 flex justify-center items-center gap-1.5 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50">
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 p-4 animate-in fade-in">
          <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl overflow-hidden scale-in animate-in zoom-in-95 duration-200">
            <div className="p-5">
              ...
              <p className="text-sm text-gray-600 mb-4">You are placing an order for <strong>{codProduct.name}</strong>. You can pay via Cash or UPI on delivery.</p>
              
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-5 font-sans">
                <div className="flex justify-between items-center mb-1 text-sm font-medium">
                  <span className="text-gray-600">Unit Price:</span>
                  <span className="text-gray-900">₹{(codProduct.price ?? 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center mb-1 text-sm font-medium">
                  <span className="text-gray-600">Quantity:</span>
                  <span className="text-gray-950 font-bold">{codProduct.quantity}</span>
                </div>
                <div className="flex justify-between items-center mb-1 text-sm font-medium border-t border-gray-200/60 pt-1.5 mt-1.5">
                  ...
                  <span className="text-[#15803d] font-bold text-base">₹{((codProduct.price ?? 0) * codProduct.quantity).toLocaleString()}</span>
                </div>
